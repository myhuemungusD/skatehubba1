 ...
 */
async function signInWithPopup(authInstance: typeof auth, provider: GoogleAuthProvider): Promise<{ user: any }> {
  if (isMockMode()) {
 ...
 */
async function signInWithRedirect(authInstance: typeof auth, provider: GoogleAuthProvider): Promise<void> {
  if (isMockMode()) {
