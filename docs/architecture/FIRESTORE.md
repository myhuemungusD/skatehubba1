...
const {
  ...
  loading,
  error,
} = useFirestoreCollection("chat_messages", [
...
const { data: game, loading } = useFirestoreDocument("game_sessions", gameId);

// Listen to active check-ins at a spot
const { data: activeUsers } = useFirestoreCollection("active_checkins", [
  where("spotId", "==", spotId),
]);
```
...
await createDocument(
  firestoreCollections.chatMessages,
  {
    userId: userId,
...
await createDocument(
  firestoreCollections.challengeVotes,
  {
    userId: auth.currentUser.uid,
